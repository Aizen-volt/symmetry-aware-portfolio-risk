...
def _replace_missing_sentinels(df: pd.DataFrame) -> pd.DataFrame:
    """
    Fama-French industry files often use -99.99 / -999 as missing sentinels.
    """
    out = df.copy()
...
def _maybe_convert_percent_to_decimal(df: pd.DataFrame) -> pd.DataFrame:
    """
    If magnitudes suggest returns are in percent units, divide by 100.
    Fama-French daily industry files are usually in percent.
    """
...
def load_ff_industry_returns(
    ...
    # engine="python" is more forgiving for legacy FF csv quirks
    df = pd.read_csv(
        path,
        index_col=0,
