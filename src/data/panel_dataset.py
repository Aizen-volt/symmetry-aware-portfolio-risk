...
class MultiAssetPanelDataset(Dataset):
    def __init__(
        ...
        self._ret = self.returns_df.to_numpy(dtype=np.float32)     # [T, N]
        self._mkt = self.market_series.to_numpy(dtype=np.float32)  # [T]
        self._wgt = self.weights_df.to_numpy(dtype=np.float32)     # [T, N]
        ...
        self._rolling_mean = self.returns_df.rolling(
            window=self.vol_lookback,
            min_periods=max(5, self.vol_lookback // 2),
    ...
    def fit_normalizer(self, indices: Optional[list[int]] = None) -> None:
        if indices is None:
    ...
    def _make_features(self, t: int) -> np.ndarray:
        sl = slice(t - self.lookback + 1, t + 1)

        ret = self._ret[sl]                    # [L, N]
        mkt = self._mkt[sl][:, None]           # [L, 1]
        vol = self._vol[sl]                    # [L, N]
        dvol = self._downside_vol[sl]          # [L, N]
        rmean = self._rolling_mean[sl]         # [L, N]

        L, N = ret.shape
        mkt_b = np.repeat(mkt, N, axis=1)

        feat = np.stack(
            [ret, mkt_b, vol, dvol, rmean],
            axis=-1,
        )  # [L, N, F]

        feat = np.transpose(feat, (1, 0, 2))  # [N, L, F]
    ...
    def _make_asset_mask(self, t: int) -> np.ndarray:
        sl = slice(t - self.lookback + 1, t + 1)
        x_window = self._ret[sl]
        mask = ~np.all(np.isnan(x_window), axis=0)
        return mask.astype(np.float32)

    def _make_weights(self, t: int, asset_mask: np.ndarray) -> np.ndarray:
        w = self._wgt[t].copy()
    ...
    def _make_target(self, t: int, weights: np.ndarray) -> float:
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        return {
            "x": torch.from_numpy(x),  # [N, L, F]
            "asset_mask": torch.from_numpy(asset_mask),  # [N]
            "weights": torch.from_numpy(weights),  # [N]
